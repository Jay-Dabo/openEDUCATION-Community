...
hoop_on=1; effects_on_mobile=1; obj2hoop=false; poststamp=0; current_status=1; freeaudio=1; usrinteaction=0
ignored_users=new Array('0'); pmnotifications=new Array('0'); uxtra_motto=new Array(); uxtra_avatar=new Array();  uxtra_expire=new Array(); phistorycached=new Array(); phistoryhidebtn=new Array();
...
function play_s(x){console.log('No audio sources...')}

if(typeof enter_ogg != 'undefined' && typeof enter_mp3 != 'undefined'){
ogg_ok=false; mp3_ok=false; audf=document.createElement('audio')

if(audf.canPlayType && ''!=audf.canPlayType('audio/ogg')){
ogg_ok=true; snd1=enter_ogg; snd2=messg_ogg; snd3=pmssg_ogg; snd4=bgmsg_ogg; }
else if(audf.canPlayType && ''!=audf.canPlayType('audio/mpeg')){
mp3_ok=true; snd1=enter_mp3; snd2=messg_mp3; snd3=pmssg_mp3; snd4=bgmsg_mp3; }


if(window.parent && typeof mobileapp!='undefined' && mobileapp>1){
function play_s(x){
if(sound_on==1 && x>0){parent.postMessage(x,'*')}}}
else{
function play_s(x){
if((mp3_ok || ogg_ok) && sound_on==1 && x>0 && freeaudio>0 && usrinteaction>0){
switch(x){
case 2: y=snd2;break
case 3: y=snd3;break
case 4: y=snd4;break
default:y=snd1}
if(mobile==1){y=snd2}
audf.src=y; freeaudio=0; audf.play(); setTimeout('freeaudio=1',100)
}}}}
...
if((effects_on_mobile<1 && mobile>0) || hoop_on<1){
...
function init(){
display_layout_rsz()
window.onresize=display_layout_rsz
ax_ping(0); ax_status(welcome_msg)
}
...
function ax_status(x){onl.innerHTML=''; log.innerHTML=x}
...
htmlmsg=msg_template.replace('{TIME}',time).replace('{GROUP}',gclr).replace('{NAME}',name).replace('{UID}',from).replace('{COLOR}',clas).replace('{TEXT}',text)

if(from<1 || room>0){
if(typeof rooms[room]=='object'){rooms[room][1]+=htmlmsg; rooms[room][2]++; if(bg_sound>0 && sound<1){sound=4}}
if(current_room==room){log.innerHTML=log.innerHTML+htmlmsg; scrolllog();if(sound<2 || sound==4){sound=2}}
recalc_msg()}
...
pmnote=msg_template.replace('{TIME}','').replace('{GROUP}',0).replace('{NAME}',lang['system']).replace('{UID}',0).replace('{COLOR}',0).replace('{TEXT}',lang['pmmsg']+' <b>'+name+'</b>')
log.innerHTML=log.innerHTML+pmnote; scrolllog(); pmnotifications.splice(1,0,pbox)}
...
pmlog.scrollTop=pmlog.scrollHeight; sound=3}
}
...
if(u>0){urn.style.display='block';urb.style.opacity=1.0}else{urn.style.display='none';urb.style.opacity=0.0}
...
for(j in ignored_users){if(uo[i][2]==ignored_users[j]){linethrough=' style="text-decoration:line-through"';}}

sortbyname=uo[i][2].toLowerCase()
onl_str.push('<!-- '+sortbyname+' --><div class="single_online_user"><span id="e'+uo[i][0]+'" class="'+ntfy+'"></span><span'+linethrough+' class="g'+uo[i][1]+'" onclick="show_user('+i+','+uo[i][0]+','+uo[i][1]+',\''+uo[i][2]+'\')">'+uo[i][2]+'</span></div>')
...
if(uxtra_motto[x] && uxtra_avatar[x]){
de('user_avatar').style.backgroundImage='url('+uxtra_avatar[x]+')'
...
inp.placeholder=' '+lang['message'];inp.readOnly=false;inp_focus()}
...
online_construct();save_cookie(ignored_cookie_name,ignored_cookie_length,JSON.stringify(ignored_users))}
...
if(autoscroll==0){ascroll();inp_focus();return}
...
switch(x){
case 0:s0.style.display='block'; s1.style.display='none'; s2.style.display='none'; s3.style.display='none'; s4.style.display='none';break
case 1:s0.style.display='none'; s1.style.display='block'; s2.style.display='none'; s3.style.display='none'; s4.style.display='none';break
case 2:s0.style.display='none'; s1.style.display='none'; s2.style.display='block'; s3.style.display='none'; s4.style.display='none';break
case 3:s0.style.display='none'; s1.style.display='none'; s2.style.display='none'; s3.style.display='block'; s4.style.display='none';break
case 4:s0.style.display='none'; s1.style.display='none'; s2.style.display='none'; s3.style.display='none'; s4.style.display='block';break
}}
...
x=de('ampm')
if(time_ampm==2){x.value='am/pm';time_ampm=1;if(u>0){settings_snd('ampm',time_ampm)} return}
if(time_ampm==1){x.value='···';  time_ampm=0;if(u>0){settings_snd('ampm',time_ampm)} return}
if(time_ampm==0){x.value=' 24H ';  time_ampm=2;if(u>0){settings_snd('ampm',time_ampm)} return}
}

// ----------

function swap_sound(u){
x=de('sndd');y=de('bgsndbox')
if(sound_on==1){y.style.display='none';x.className='panel_button x_bcolor_z x_all_rounded svg_soff';sound_on=0;if(u>0){settings_snd('sound',sound_on)} return}
if(sound_on==0){y.style.display='block';x.className='panel_button x_bcolor_z x_all_rounded svg_sndd';sound_on=1;if(u>0){settings_snd('sound',sound_on)} play_s(1); return}
}

// ----------

function swap_bgsnd(u){
x=de('bgsnd')
if(bg_sound==1){x.className='panel_button x_bcolor_z x_all_rounded svg_soff';bg_sound=0;if(u>0){settings_snd('bgsnd',bg_sound)} return}
if(bg_sound==0){x.className='panel_button x_bcolor_z x_all_rounded svg_sndd';bg_sound=1;if(u>0){settings_snd('bgsnd',bg_sound)} play_s(4); return}
}
...
if(u>0){
settings_snd('color',current_color)
hst.style.display='none';pab.style.display='none';hoop(pan,0);inp_focus()}
}

// ----------

function settings_snd(x,y){
ajax_stt=new XMLHttpRequest()
x=x+'='+y
ajax_stt.open('post','index.php')
ajax_stt.setRequestHeader('Content-Type','application/x-www-form-urlencoded')
ajax_stt.onreadystatechange=settings_ans;ajax_stt.send(x)}

function settings_ans(){
if(ajax_stt.readyState==4 && ajax_stt.status==200){response=ajax_stt.responseText.toString()
if(response.length>1){s=self.location.href.toString();gourl(s)}
}}
...
function ascroll(){if(autoscroll==0){hoop(aus,0);autoscroll=1;scrolllog();inp_focus()}}

function mdown(x){if(allowselect<1 && typeof x.preventDefault=='function'){
x.preventDefault()}dragdown=true;dragypos=x.pageY}

function mmove(x,y){if(dragdown==true && allowselect<1 && dragypos){
x.scrollTop+=((dragypos-y.pageY)/6)
if(autoscroll>0 && dragdown==true){hoop(aus,1);autoscroll=0}}}

function wmove(e){if(parseInt(e.deltaY)>0){z=15}else{z=-15}
log.scrollTop+=z; if(autoscroll>0){hoop(aus,1);autoscroll=0}}

function keyscroll(m){
if(autoscroll>0 && (m==38 || m==40)){hoop(aus,1);autoscroll=0;}
...
function ttouch2(e){
tj = e.changedTouches[0]
currx = parseInt(tj.clientX)
curry = parseInt(tj.clientY)
if((curry-globy)>50 || (globy-curry)>50){if(autoscroll>0){autoscroll=0;hoop(aus,1)}}
else if(globx<50 && (currx-globx)>50){hst.style.display='block';pab.style.display='block';hoop(pan,1)}
}
...
theurl='msgfetch.php?tuid='+ext_usr_id+'&mtoken='+mtoken+'&tpoi='+tpoint+'&zone='+zone+'&ampm='+time_ampm
...
msgft=msg_template.replace('{TIME}',h_time).replace('{NAME}',h_uname).replace('{UID}',h_usrid).replace('{COLOR}',h_color).replace('{TEXT}',h_text)
...
function save_cookie(cname,clength,cvalue){
var expdate = new Date(); expdate.setTime(expdate.getTime() + clength*1000);
expdate = "; expires=" + expdate.toUTCString();cvalue=encodeURIComponent(cvalue);
