...
$lang_divs='';
foreach ($lang_list as $key => $value) { $lang_divs.='<div title="'.$value[1].'" class="svg_f_'.$value[0].'" onclick="gourl(\'index.php?lang='.$key.'\')"></div>'."\n"; }
...
$info_url='account.php'; $info_line=$lang['no_upmatch'];
require_once('templates/info.pxtm'); die();}
...
$info_url='account.php?passrequired_uname='.$postname; $info_line=$lang['no_upmatch'];
require_once('templates/info.pxtm'); die();}

if($dbuser['quarantine']>$timestamp){
$info_url='account.php?passrequired_uname='.$postname; $info_line=$lang['account_wait'];
require_once('templates/info.pxtm'); die();}
...
$info_url='account.php?passrequired_uname='.$postname; $info_line=$lang['pass_required'];
require_once('templates/info.pxtm'); die();}
...
$info_url='account.php'; $info_line=$lang['guest_error'];
require_once('templates/info.pxtm'); die();}
...
$info_url='account.php?q=register'; $info_line=$lang['wrong_usr_ps'];
require_once('templates/info.pxtm'); die();}

if(length($regmail)<7 || !stristr($regmail,'@') || !stristr($regmail,'.') || stristr($regmail,' ')){
$info_url='account.php?q=register'; $info_line=$lang['wrong_email'];
require_once('templates/info.pxtm'); die();}

if(length($reguery)<1 || length($_POST['answer'])<1){
$info_url='account.php?q=register'; $info_line=$lang['wrong_quas'];
require_once('templates/info.pxtm'); die();}

$res=neutral_query('SELECT * FROM '.$dbss['prfx']."_users WHERE name='$regname' OR email='$regmail'");

if(neutral_num_rows($res)>0){
$info_url='account.php?q=register'; $info_line=$lang['wrong_usr_em'];
require_once('templates/info.pxtm'); die();}
...
$info_url='account.php?passrequired_uname='.$regname; $info_line=$lang['account_ok'];
...
$info_url='account.php?q=password'; $info_line=$lang['wrong_recmail'];
require_once('templates/info.pxtm'); die();}
...
$info_url='account.php?q=password'; $info_line=$lang['wrong_recansw'];
require_once('templates/info.pxtm'); die();}
...
$info_url='account.php?passrequired_uname'; $info_line=$lang['pass_newpass'].'<b>'.$pass.'</b>';
require_once('templates/info.pxtm'); die();
